...
def ReduceMasterFile(df, meld_cols, end_date, age_min = 18, age_max = 120, exc_cands = 'Exclude', status1A_cands = 'Exclude', status1B_cands = 'Exclude'):
    ...
    #Finally, remove any rows for which th canhx_begin_dt == canhx_end_dt == rec_tx_dt.
    #UNLESS THE CANHX_BEGIN_DT IS THE SAME AS THE LISTING DATE!. These are status updates
    #that were created when the candidate came in for transplant AFTER allocation occured. For that reason they are not
    #informative
    df = df.loc[~((df['canhx_begin_dt'] == df['canhx_end_dt']) & (df['canhx_begin_dt'] == df['rec_tx_dt']) & (df['canhx_begin_dt'] != df['can_listing_dt']))]
